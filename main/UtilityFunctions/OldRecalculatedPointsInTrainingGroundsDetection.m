function [boolean_points_in_training_ground, objectcases_in_training_ground, objectcases_under_ceil, ...
    objectcases_total] = RecalculatedPointsInTrainingGroundsDetection(RecalculatedPoints, ...
    input_json_txtTrainingGround, training_groud_ceil)

objectcases_in_training_ground = 0;
objectcases_under_ceil = 0;
objectcases_total = 0;
objectcases_total2 = 0; % to delete later
...
for i = 1:length(fieldnames_A)
    for j = 1:length(RecalculatedPoints.Apogee.(fieldnames_LP{i, 1}).lat)
        if RecalculatedPoints.Apogee.(fieldnames_A{i, 1}).alt(j) < training_groud_ceil
            objectcases_under_ceil = objectcases_under_ceil + 1;
        elseif isnan(RecalculatedPoints.Apogee.(fieldnames_LP{i, 1}).alt(j))
            objectcases_nans_2 = objectcases_nans_2 + 1;
        end
        objectcases_total2 = objectcases_total2 + 1; % to delete later
    end
end
