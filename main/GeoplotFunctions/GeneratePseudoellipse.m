function [lat_ellipse, long_ellipse] = GeneratePseudoellipse(lat_launchpad, long_launchpad, frontwind_point, ...
    backwind_point, sidewind_point, boolean_draw_ellipse)
...
[lat_second_side, long_second_side] = CalculateSecondSideWindLandingPoint(lat_launchpad*180/pi, ...
        long_launchpad*180/pi, frontwind_point(1), frontwind_point(2), sidewind_point(1), sidewind_point(2));
...
front_azimuth = atan2(ned_front(2),ned_front(1));

% calculating points for 100 lines for pseudoellipse
theta = linspace(0,2*pi,100)';
% formulas inside ifs differenciate by min and max inside first log and
% formulas have two versions simple one with theta in cos and complicated 
% one with 2*theta in cos 
if front_dist > side_dist && side_dist > back_dist % f>s>b
    ellipse_dist = back_dist+(front_dist-back_dist)*((1+cos(theta))/2).^ ...
        (log(min(abs(front_dist-side_dist),abs(side_dist-back_dist))/abs(front_dist-back_dist)) / log((1+cos(pi/2))/2));
elseif front_dist < side_dist && side_dist < back_dist % b>s>f
    ellipse_dist = back_dist+(front_dist-back_dist)*((1+cos(theta))/2).^ ...
        (log(max(abs(front_dist-side_dist),abs(side_dist-back_dist))/abs(front_dist-back_dist)) / log((1+cos(pi/2))/2));
elseif front_dist > back_dist && back_dist > side_dist % f>b>s
    ellipse_dist = [side_dist + (front_dist-side_dist) * ((1+cos(2*theta(1:25)))/2).^ ...
        (log(min(abs(front_dist-back_dist),abs(back_dist-side_dist))/abs(front_dist-back_dist)) / log((1+cos(pi/2))/2)); ...
        side_dist + (back_dist-side_dist) * ((1+cos(2*theta(26:75)))/2).^ ...
        (log(min(abs(front_dist-back_dist),abs(back_dist-side_dist))/abs(front_dist-back_dist)) / log((1+cos(pi/2))/2)); ...
        side_dist + (front_dist-side_dist) * ((1+cos(2*theta(76:100)))/2).^ ...
        (log(min(abs(front_dist-back_dist),abs(back_dist-side_dist))/abs(front_dist-back_dist)) / log((1+cos(pi/2))/2))];
elseif front_dist < back_dist && back_dist < side_dist % s>b>f
    ellipse_dist = [side_dist + (front_dist-side_dist) * ((1+cos(2*theta(1:25)))/2).^ ...
        (log(max(abs(front_dist-back_dist),abs(back_dist-side_dist))/abs(front_dist-back_dist)) / log((1+cos(pi/2))/2)); ...
        side_dist + (back_dist-side_dist) * ((1+cos(2*theta(26:75)))/2).^ ...
        (log(max(abs(front_dist-back_dist),abs(back_dist-side_dist))/abs(front_dist-back_dist)) / log((1+cos(pi/2))/2)); ...
        side_dist + (front_dist-side_dist) * ((1+cos(2*theta(76:100)))/2).^ ...
        (log(max(abs(front_dist-back_dist),abs(back_dist-side_dist))/abs(front_dist-back_dist)) / log((1+cos(pi/2))/2))];
elseif back_dist > front_dist && front_dist > side_dist % b>f>s
    ellipse_dist = [side_dist + (front_dist-side_dist) * ((1+cos(2*theta(1:25)))/2).^ ...
        (log(min(abs(back_dist-front_dist),abs(front_dist-side_dist))/abs(back_dist-side_dist)) / log((1+cos(pi/2))/2)); ...
        side_dist + (back_dist-side_dist) * ((1+cos(2*theta(26:75)))/2).^ ...
        (log(min(abs(back_dist-front_dist),abs(front_dist-side_dist))/abs(back_dist-side_dist)) / log((1+cos(pi/2))/2)); ...
        side_dist + (front_dist-side_dist) * ((1+cos(2*theta(76:100)))/2).^ ...
        (log(min(abs(back_dist-front_dist),abs(front_dist-side_dist))/abs(back_dist-side_dist)) / log((1+cos(pi/2))/2))];
    ...
    ellipse_dist = [side_dist + (front_dist-side_dist) * ((1+cos(2*theta(1:25)))/2).^ ...
        (log(max(abs(back_dist-front_dist),abs(front_dist-side_dist))/abs(back_dist-side_dist)) / log((1+cos(pi/2))/2)); ...
        side_dist + (back_dist-side_dist) * ((1+cos(2*theta(26:75)))/2).^ ...
        (log(max(abs(back_dist-front_dist),abs(front_dist-side_dist))/abs(back_dist-side_dist)) / log((1+cos(pi/2))/2)); ...
        side_dist + (front_dist-side_dist) * ((1+cos(2*theta(76:100)))/2).^ ...
        (log(max(abs(back_dist-front_dist),abs(front_dist-side_dist))/abs(back_dist-side_dist)) / log((1+cos(pi/2))/2))];
...
if boolean_draw_ellipse
    geoplot (lat_ellipse, long_ellipse, 'y-');
end

lat_ellipse = lat_ellipse*pi/180;
long_ellipse = long_ellipse*pi/180;
